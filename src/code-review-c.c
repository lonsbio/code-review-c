...
void printUsage() {
    puts("Synopsis:\n"
         "  Print fasta stats\n"
         "Usage:\n"
         "  code-review-c [options] contigs.fasta [another.fa ...]\n"
         "Options:\n"
         "  --help         Show this help\n"
 ...
 */
void printVersion() {
    printf("code-review-c version %s\n", VERSION);
}
...
int processFiles(FILE *logFile, int minlen, char **files, int fileCount) {
    code-review-c_log(logFile, "code-review-c version %s starting.", VERSION);
    code-review-c_log(logFile, "processing %i file(s) with minlen %i...", fileCount, minlen);
    puts(HEADER);
    if (fileCount == 0) {
        code-review-c_log(logFile, "reading stdin...");
        ...
        code-review-c_log(logFile, "reading stdin: done");
    }
    else {
        code-review-c_log(logFile, "reading %i file(s)...", fileCount);
        for (int current = 0; current < fileCount; current++) {
            code-review-c_log(logFile, "reading %s...", files[current]);
            ...
            code-review-c_log(logFile, "reading %s: done", files[current]);
        }
        code-review-c_log(logFile, "reading %i file(s): done", fileCount);
    }
    return EXIT_OK;
}
